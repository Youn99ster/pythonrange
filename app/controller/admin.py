...
def dashboard():
    ...
    recent_orders = []
    for o in recent:
        label, cls = get_order_status_meta(o.payment_status)
        product_name = ""
        product_quantity = 0
        try:
            if o.items:
...
def vouchers():
    page = request.args.get("page", 1, type=int)
    per_page = 10
    pagination = Voucher.query.order_by(Voucher.created_at.desc()).paginate(page=page, per_page=per_page, error_out=False)
...
def generate_vouchers():
    ...
    try:
        db.session.commit()
        flash(f"成功生成 {generated_count} 张储值券，面额 ¥{amount}", "success")
...
def products_batch_import():
    ...
    try:
        # SSRF：不做校验直接请求管理员提交的 URL。
        with urllib.request.urlopen(url) as resp:
            data_bytes = resp.read()
    except Exception as e:
        return jsonify({"success": False, "error": f"文件获取失败：{str(e)}"}), 400
