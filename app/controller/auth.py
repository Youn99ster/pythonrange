...
def register():
    ...
    if password != confirm_password:
        return jsonify({"status": "error", "message": "两次密码输入不一致"}), 400
...
def forgot_password():
    if request.method == "GET":
        return render_template(
            "auth/forgot-password.html", data={"page": "forgot-password page"}
        )
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"status": "error", "message": f"重置邮件发送失败: {e}"}), 500
...
def reset_password(token):
    ...
    user_id = redis_client.get(f"reset_token:{token}")
    if not user_id:
        return jsonify({"status": "error", "message": "链接已失效或过期"}), 400
